...
roles = {}
for i in range(r):
    name, num, role_cates = input().split(' ', 2)   #分成三份
    quanxian = []   #用来记录该角色的所有权限
    for role_cate in role_cates.split():
        role_cate = role_cate.split(':')
        if len(role_cate) == 1:
...
users = {}
for i in range(u):
    user, num, user_roles = input().split(' ', 2)
    user_quanxian = {}   #用来存放该用户的所有权限
    for user_role in user_roles.split():
        quanxian_s = roles[user_role]   #得到该角色的权限列表
